...
| number of items: n | cost of lookup: O(log n) |
|--------------------+--------------------------|
| one thousand       | 10 * k                   |
| one million        | 20 * k                   |
| one billion        | 30 * k                   |
...
A fun warmup for metaprogramming is writing quines, but most quines, including the heroic PolyQuines in 
[[https://www.youtube.com/watch?v=6avJHaC3C2U][The Art of Code]] do not demonstrate the kind of metaprogramming you like.
...
1. Writing a script or program which reads in some data and then writes a new
   script or program which you can run later. Compilers are an example of this
   kind of metaprogramming. So are the horribly complex but powerful [[https://www.gnu.org/software/automake/manual/html_node/Autotools-Introduction.html][GNU
   Autotools]] which are used to configure many complex software systems before
   building and installing them. This technique is /heavyweight/ as it involves
   (1) running program or script (A) which does I/O reading the specifications
   (usually form a file) and writes the new script or program (usually to
   another file) and then arranges for the resulting output file to be set up
   for execution at the appropriate time. In addition to the I/O overhead, the
   specifications have to be parsed and verified and the code which is generated
   will need to be parsed, verified and translated (compiled or interpreted)
   into machine language. We're talking massive overhead. Even on today's
   computers this is often something which takes many minutes to run.
2. All scripting languages and many compiled languages have the ability to dynamically
   parse and execute code which is in the form of a data structure in memory.  In most
   languages this will simply be a string, but in homoiconic languages such as Lisp and 
   Prolog we can use symbolic expressions to more naturally express desired code.
   This means we can have functions output such strings or symbolic expressions
   into memory and then tell a built-in compiler or interpreter to immediately
   parse and translate them into efficient code which can be run immediately or 
   at a later time.  This is a similar but more lightweight version of the first
...
3. Languages which have fully-general lambda functions, originally just Lisps, but 
   recently appearing (not fully-general, but still very powerful) in newer versions
   of Python, Java, Javascript, Microsoft Excel, etc. can do a much more efficient 
   and natural form of metaprogramming.  An ordinary function can simply return a
   Lambda function which can then be immediately excuted if suitable prameters are 
   already available, or it can be bound to a name for use and reuse later in the 
   program.  In Lisp /all functions are actually Lambda functions/ and in Lisp 
   functions are first class data structures which can be sent over network channels
   to be excuted remotely, stored in databases and files, etc.  When Lambda Functions
   are fully integrated into the compiler, as they are in Lisp, they don't require 
   any new parsing or analysis - they are almost instantly available as machine code 
   and they're just as reliable and efficient as any other functions.
...
single very long line. It's incomplete.  We are not impressed!
...
permissions. Pretty ugly, though!
...
This Scheme quine avoids the parsing headache but still needs to be translated
...
Lambda syntax is the => operator. There are limitations preventing it from being
...
It has also been a disaster for the field as a whole because it has become a
cult, it is being touted as a panacea. Gullible people are being informed that
Machine Learning can always substitute for actual understanding or that
understanding will mysteriously /emerge/ from the neural networks. Alas, it
doesn't work that way! Machine Learning systems are extremely fragile. For most
tasks they need to be integrated with more traditional techniques which actually
model what's going on in the domain and check that the pattern matching
generated by the neural networks makes sense. If you'd like to learn more about
this I recommend the easy-to-read popularization [[https://www.goodreads.com/book/show/43999120-rebooting-ai][Rebooting AI: Building
