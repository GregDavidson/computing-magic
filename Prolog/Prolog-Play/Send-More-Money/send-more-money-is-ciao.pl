% SEND + MORE = MONEY -- using non-logical arithmetic

% Tested using Ciao Prolog
% Using built-in predicates
% is/2 - non-logical arithmetic
% delete/3 - L1 w/o X is L2
...
% List should be ground or use delete_non_ground instead.
perm(List,[H|Perm]):-delete(List,H,Rest),perm(Rest,Perm).
perm([],[]).

%% *** add single digits with Sum and Carry

% X and Y should be instantiated as single-digit integers
add(X, Y, Sum, Carry) :-
    Total is X + Y,
    Sum is Total mod 10,
    Carry is Total // 10. % // was div in SWI-Prolog
