#lang racket
;; * Flight Lander Game (2htdp, Lang Racket, Functional)

;; ** Overview

;; Functional Programming Paradigm used throughout.
;; All data structures are immutable values.
...
(struct	simple-sprite (image x y)
  #:transparent
  #:guard  (λ (image x y name) ; check that our field values are valid
             (values (the-image image '(simple-spritel image))
                     (the-natural x '(simple-sprite x))
                     (the-natural y '(simple-sprite y)) ) ) )
...
;; Ideally would check it's of type (-> sprite sprite)
;; i.e. a procedure which takes a sprite and returns a sprite
...
(struct	sprite simple-sprite (dx dy update)
  #:transparent
  #:guard  (λ (image x y dx dy update name)
             (values (the-image image '(sprite image))
                     (the-natural x '(sprite x))
                     (the-natural y '(sprite y))
                     (the-integer dx '(sprite dx))
                     (the-integer dy '(sprite dy))
                     (the-update-function update '(sprite update)) ) ) )

;; *** abstract-sprite and update-sprite
...
;; Return a (possibly new) sprite based on an existing-sprite
;; and the specified field values.  If the specified field values
;; are different from the values in the existing-sprite, create
;; a new sprite, otherwise return the existing-sprite.
;; The existing-sprite could be a simple-sprite, but we will
;; always return an extended sprite.
(define (update-sprite existing-sprite
  ...
  (let ( ; simple-sprite fields
        [old-image (sprite-image existing-sprite)]
        [old-x (sprite-left existing-sprite)]
        [old-y (sprite-bottom existing-sprite)]
        ; extended sprite fields
        [old-dx (and (sprite? existing-sprite) (sprite-dx existing-sprite))]
        [old-dy (and (sprite? existing-sprite) (sprite-dy existing-sprite))]
        [old-update (and (sprite? existing-sprite) (sprite-update existing-sprite))] )
    (let ( ; desired sprite fields
          [image (or new-image old-image)]
          [x (or new-x old-x)]
          [y (or new-y old-y)]
          [dx (or new-dx old-dx)]
          [dy (or new-dy old-dy)]
          [update (or new-update old-update)] )
      (let ( ; will any fields change?
        ...
        (if (and same (sprite? existing-sprite))
            existing-sprite             ; keep the existing sprite
            (sprite image x y dx dy update) ) ) ) ) )
...
;; Return a sprite with x and y updated based on dx and dy
...
(define PLANE-SUNK (update-sprite PLANE #:x 0 #:y SCENE-HEIGHT))
...
(define PLANE-BARELY-LANDED             ; whew!
  (update-sprite PLANE-ALMOST-LANDED #:x WATER-WIDTH) )
...
(define PLANE-AT-LANDS-END ; landed plane at the end of the land
  (update-sprite PLANE-BARELY-LANDED
  ...
  (let* ( [plane (car world)]
    ...
    (if (= y new-y)
        ...
        (begin
          ...
          (cons (update-sprite plane #:y new-y) (cdr world)) ) ) ) )
