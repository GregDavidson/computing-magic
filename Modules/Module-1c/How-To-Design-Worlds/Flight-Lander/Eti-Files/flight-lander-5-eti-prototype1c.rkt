...
;; See the on-tick method attached to the plane!
...
(define (move-plane-on-tick sprite)
  (let ( [x (sprite-x sprite)] [y (sprite-y sprite)] )
    (cond
      ...
      [(and(over-water? sprite) (over-land? sprite)) sprite]
      ;; if in water you cant move fast
      [(over-water? sprite) (move-sprite-dx-dy sprite 0 PLANE-IN-WATER-MOVE-Y)]
      ;;if on land and we're supposed to move we need to be doing so
      [(over-land? sprite) (move-sprite-dx-dy sprite PLANE-ON-LAND-MOVE-X 0)]
      ;;were lost
...
(define sprite-magazine
  (list (make-sprite PLANE 0 0 PLANE-MOVE-X PLANE-MOVE-Y move-plane-on-tick)
