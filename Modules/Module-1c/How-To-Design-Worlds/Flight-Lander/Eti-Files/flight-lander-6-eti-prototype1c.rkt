...
;; EXERCISES:
;; Refine the fraction contracts
;; to specify rationals within [0..1)
...
(define (hotbox-x2 hb)
  (let* ( [s-w (sprite-width (hotbox-sprite hb))]
          ...
          [hb-x (hotbox-x hb)]
          [hb-x2- (+ hb-x hb-w-)] ; untrimmed top edge
          )
    (min hb-x2- s-w) ) )
...
(define disker-x-robot (new-sprite ROBOT-RIGHT
                     (- WIDTH (image-width ROBOT-RIGHT) MARGIN) 400) )
(define disker-x-m-launcher (hotbox 0 3/4 1/10 1/10 disker-x-robot))
...
(define gatling-missile
  (new-sprite G-MISSILE (random WIDTH) (random HEIGHT)
              #:dx -2 #:dy -5 #:draw gatling-fire ) )
(define missile-tip (hotbox 0 0 1/10 1/10 gatling-missile))
...
(define (game-over? world)
  (or (on-bottom?  plane)
      (sprite-overlap plane gatling-missile)
