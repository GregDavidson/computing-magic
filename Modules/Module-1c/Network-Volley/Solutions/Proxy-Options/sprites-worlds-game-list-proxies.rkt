...
(define (make-welcome n . alist)
  (unless (natural? n) (error 'make-welcome "invalid world-number ~a" n))
  (unless (symbol-key-alist? alist) (error 'make-welcome "invalid alist ~a" alist))
  (cons U2W-WELCOME (cons (list WORLD-NUMBER-KEY n) alist)) )

(define (welcome-alist welcome)
  (unless (welcome? welcome) (error 'welcome-alist "invalid welcome ~a" welcome))
  (cdr welcome) )

(define (welcome-world-number welcome)
  (let ( [found (assoc WORLD-NUMBER-KEY (welcome-alist welcome))] )
    (unless (and (pair? found) (= (length found) 2)) (error 'welcome-world-number "missing world number in welcome ~a" welcome))
    (let ( [number (second found)] )
      (unless (natural? number) (error 'welcome-world-number "invalid world number ~a in ~a" number welcome))
      number ) ) )
...
(define (sprite-proxy uuid image x y dx dy on-tick on-key to-draw)
  ;; put guard here!!!
  ;; add type tag??
  (list uuid image x y dx dy on-tick on-key to-draw) )

(define (sprite-proxy? something)
  ;; put in a more stringent test later!!
  (and (pair? something) (strict-uuid-string? (car something))) )

(define sprite-proxy-uuid first)
(define sprite-proxy-image second)
(define sprite-proxy-x third)
(define sprite-proxy-y fourth)
(define sprite-proxy-dx fifth)
(define sprite-proxy-dy sixth)
(define sprite-proxy-on-tick seventh)
(define sprite-proxy-on-key eighth)
(define sprite-proxy-to-draw ninth)
