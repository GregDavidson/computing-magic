...
** Step 1: Draw an Eye Interactively
...
** Step 2: Draw Using A Parameterized Procedure
...
#+begin_src racket
  (define (draw-eye YOUR PARAMETERS HERE)
    (cc-superimpose
...
#+begin_src racket
  (define (draw-eye eye-height eye-ratio iris-ratio pupil-ratio iris-color)
...
#+begin_src racket
  (draw-eye 100)
  (draw-eye 100 #:color "brown" #:ratio 2 #:pupil 1/5)
#+end_src
...
#+begin_src racket
  (define (draw-eye eye-height #:ratio [eye-ratio 2]
...
** Step 4: Build up to Drawing A Face
...
Suggested flow:
- Create a procedure for drawing two eyes.
      - optional first argument of append functions specifies spacing
- Create a procedure for drawing a nose
      - make it simple for now, e.g. a disk
- Create a procedure for drawing eyes+nose
      - Calling your earlier two procedures
- Create a procedure for drawing a mouth
      - make it simple for now, e.g. a filled ellipse
- Create a procedure for drawing eyes+nose+mouth
      - Calling your earlier two procedures
...
#+begin_src racket
  ...
  (define (draw-eye eye-height #:ratio [eye-ratio eye-ratio]
...
#+begin_src racket
  ...
  (define (draw-eyes eye-height
                     ...
                     #:iris [iris-ratio eye-iris-ratio] #:pupil [pupil-ratio eye-iris-pupil-ratio]
                     #:color [iris-color eye-color] #:sclera-color [sclera-color eye-sclera-color] )
    (let ([eye (draw-eye eye-height #:ratio eye-ratio #:iris iris-ratio #:pupil pupil-ratio
                         #:color iris-color #:sclera-color sclera-color )])
      (hc-append (* spacing-ratio eye-ratio eye-height) eye eye) ) )

  (define (draw-nose size) (disk size))

  (define (draw-eyes+nose face-height
    ...
    (printf "~a\n" `(vc-append ,eye-nose-spacing
                               (draw-eyes ,(* face-height face-eye-ratio))
                               (draw-nose ,(* face-height face-nose-ratio)) ))
    (vc-append eye-nose-spacing
               (draw-eyes (* face-height face-eye-ratio)
  ...
  (define (draw-mouth width #:ratio [ratio mouth-ratio] #:color [color mouth-color])
    (filled-ellipse width (* ratio width) #:color color) )

  (define (draw-eyes+nose+mouth face-height
    ...
    (let* ( [spacing (* nose-mouth-ratio face-height)] )
      (vc-append spacing
                 ...
                 (draw-mouth (* face-height face-mouth-ratio) #:ratio face-mouth-ratio #:color mouth-color) ) ) )

  (define (draw-face height  #:ratio [ratio face-ratio]
    ...
    (let* ( [width (* ratio height)]
            [content (draw-eyes+nose+mouth height
...
** Step Infinity: Drawing Complex Objects, e.g. Noses, Mouths, Eyelids, etc.

Once you determine what you want to be able to draw you can select the right
...
#+begin_src racket
  ...
  (draw-eye 30 #:color (light "aquamarine"))
#+end_src
...
Racket's high-level libraries are based on the lower level /Racket Drawing
Toolkit/. If you find the existing high-level libraries too limited for
...
#+begin_src racket
  ...
  (define (draw-fancy-triangle w h)
    (dc (λ (dc dx dy)
  ...
  ;; try: (draw-fancy-triangle 50 50)
#+end_src

Notice that we're still using our familiar pict library so we can mix and match
high-level pict functions with the low-level Racket Drawing Tool features.

** Bitmaps

The Racket Drawing Library can create and manipulate Bitmaps directly.
...
The Racket Drawing Library can also work with SVGs which can be scaled to fit
any resolution.

The Racket Drawing Tookit can use SVGs by using the class svg-dc%
https://docs.racket-lang.org/draw/svg-dc_.html

The drawing methods are the same as drawing on a bitmap, but the result is often
nicer.
